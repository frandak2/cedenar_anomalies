...
# Importamos las funciones de paths
from cedenar_anomalies.utils.paths import data_processed_dir, reports_dir


class DataCleaner:
    """
    Clase para limpiar y transformar datos después del procesamiento inicial.
    
    Esta clase toma un DataFrame generado por DataProcessor y realiza operaciones
    de limpieza como tratamiento de valores nulos, eliminación de duplicados, y
    ...
    def __init__(
        ...
        """
        Inicializa el DataCleaner con el DataFrame a limpiar.
        
        Args:
            df: DataFrame de entrada que requiere limpieza
            output_filename: Nombre del archivo donde se guardará el resultado
    ...
    def generate_initial_report(self) -> Dict:
        ...
        self.logger.info(f"DataFrame original: {self.original_shape[0]} filas, {self.original_shape[1]} columnas")
    ...
    def _remove_null_coordinates(self, coordinate_columns: List[str] = ["LATI_USU"]) -> pd.DataFrame:
        """
        Elimina registros que tienen valores nulos en las columnas de coordenadas especificadas.
        ...
        """
        self.logger.info(f"Eliminando registros con coordenadas nulas en: {coordinate_columns}")
        ...
        removed_percentage = round((removed_count / original_count) * 100, 2) if original_count > 0 else 0

        self.logger.info(f"Se eliminaron {removed_count} registros con coordenadas nulas ({removed_percentage}%)")
    ...
    def _handle_boolean_columns(self, boolean_columns: List[str]) -> pd.DataFrame:
        ...
        for column in boolean_columns:
            if column in self.df.columns:
                ...
                if nan_count > 0:
                    self.logger.info(f"Reemplazando {nan_count} valores NaN con False en {column}")
                    self.df[column] = self.df[column].replace(np.nan, False)
                    
                # Convertir 'TRUE'/'FALSE' a True/False si existen
                if self.df[column].dtype == 'object':
                    self.df[column] = self.df[column].replace({"TRUE": True, "FALSE": False})
                    
        return self.df
    
    def _handle_categorical_columns(self, categorical_columns: Dict[str, str]) -> pd.DataFrame:
        """
        ...
        for column, default_value in categorical_columns.items():
            if column in self.df.columns:
                nan_count = self.df[column].isna().sum()
                if nan_count > 0:
                    self.logger.info(f"Reemplazando {nan_count} valores NaN con '{default_value}' en {column}")
                    self.df[column] = self.df[column].replace(np.nan, default_value)
                    
        return self.df
    
    def _handle_binary_indicators(self, binary_columns: Dict[str, Dict[str, str]]) -> pd.DataFrame:
        """
        ...
        for column, replacements in binary_columns.items():
            if column in self.df.columns:
                ...
                self.logger.info(f"Columna {column} convertida a formato binario: {self.df[column].value_counts().to_dict()}")
    ...
    def _mark_recurrences(self, id_column: str = "item_288") -> pd.DataFrame:
        ...
        if id_column in self.df.columns:
            ...
            self.logger.info(f"Se marcaron {reincidentes_count} registros como reincidentes")
            ...
            self.logger.warning(f"La columna {id_column} no existe en el DataFrame. No se pueden marcar reincidencias.")
    ...
    def _create_anomaly_column(self) -> pd.DataFrame:
        ...
        if "Descripcion" in self.df.columns:
            ...
            self.logger.info(f"Se clasificaron {anomalia_count} registros como 'Anomalia' y {no_anomalia_count} como 'No anomalia'")

            # Guardar estadísticas en el reporte
            self.report["anomaly_classification"] = {
                "anomalias": anomalia_count,
                "no_anomalias": no_anomalia_count,
                "porcentaje_anomalias": round((anomalia_count / len(self.df)) * 100, 2) if len(self.df) > 0 else 0
            }
        else:
            self.logger.warning("La columna 'Descripcion' no existe en el DataFrame. No se puede crear la clasificación de anomalías.")
    ...
    def _select_columns(self) -> pd.DataFrame:
        ...
        """
        self.logger.info(f"Seleccionando {len(self.columns_to_keep)} columnas para el DataFrame final")
        
        # Verificar que todas las columnas existan
        missing_columns = [col for col in self.columns_to_keep if col not in self.df.columns]
        if missing_columns:
            self.logger.warning(f"Las siguientes columnas no existen en el DataFrame: {missing_columns}")
    ...
    def generate_final_report(self) -> Dict:
        ...
        final_report["reduction"] = {
            "rows": rows_reduction,
            "rows_percentage": round((rows_reduction / self.original_shape[0]) * 100, 2) if self.original_shape[0] > 0 else 0,
            "columns": cols_reduction,
            "columns_percentage": round((cols_reduction / self.original_shape[1]) * 100, 2) if self.original_shape[1] > 0 else 0
        }
        
        self.logger.info(f"DataFrame final: {self.df.shape[0]} filas, {self.df.shape[1]} columnas")
        self.logger.info(f"Reducción: {rows_reduction} filas ({final_report['reduction']['rows_percentage']}%), "
                        f"{cols_reduction} columnas ({final_report['reduction']['columns_percentage']}%)")
    ...
    def save_results(self, output_dir: Optional[str] = None) -> str:
        """
        Guarda el DataFrame limpio y el reporte en archivos.
        
        Args:
            output_dir: Directorio opcional donde guardar los archivos (se espera que sea
                        una ruta completa o None para usar la carpeta actual)
            
        Returns:
            Ruta del archivo CSV guardado
        """
        # Si no se especifica un directorio, usar el directorio de datos procesados
        if output_dir is None:
            output_path = data_processed_dir(self.output_filename)
        else:
            # Si se especifica, construir la ruta completa
            output_path = Path(output_dir) / self.output_filename
        
        # Guardar el DataFrame limpio
        self.logger.info(f"Guardando DataFrame limpio en: {output_path}")
        self.df.to_csv(output_path, index=False)
        
        # Construir la ruta para el reporte (mismo directorio, diferente extensión)
        report_filename = self.output_filename.replace('.csv', '_report.json')
        if output_dir is None:
        ...
        # Guardar el reporte
        pd.Series(self.report).to_json(report_path)
